<form class="w70 px10 mt30" method="POST" action="<?= URL::current() ?>">
	...
	if( INPUT::formSubmitted() ) {
		require get_dashboard_path('controller/settings/seo.php');
	}
	...
	<h3 class="txt_center">
		<span class="border-span">Verificações para mecanismos de busca</span>
	</h3>
	<hr />
	<div class="flexbox">
	...
	<textarea id="home-dscpt" class="md no_enter" name="homepage_description" minlength="50" maxlength="220" placeholder="Mínimo 50 e máximo 220 caracteres"><?= $ensure('homepage_description') ?></textarea>
	...
	<hr>
	<?= $alert['posts_description'] ?? null ?>
	<label for="posts-dscpt" class="mt30 dblock">
        Descrição da página de listagem de posts e posts por pesquisa
    </label>
	<textarea id="posts-dscpt" class="md no_enter" name="posts_description" minlength="50" maxlength="220" placeholder="Mínimo 50 e máximo 220 caracteres"><?= $ensure('posts_description') ?></textarea>
	<button type="submit" class="btn dblock right mt20 w20" name="action" value="posts_action">SALVAR</button>
	<hr>
    ...
    <textarea id="cats-dscpt" class="md no_enter" name="categories_description" minlength="50" maxlength="220" placeholder="Mínimo 50 e máximo 220 caracteres"><?= $ensure('categories_description') ?></textarea>
	...
	<textarea id="user-dscpt" class="md no_enter" name="user_description" minlength="50" maxlength="220" placeholder="Mínimo 50 e máximo 220 caracteres"><?= $ensure('user_description') ?></textarea>
	<button type="submit" class="btn dblock right mt20 w20" name="action" value="user_action">SALVAR</button>
</form>
<div class="mt35 w70">
	<hr>
	<p class="mt25 fs15 italic">Para garantir a melhor exibição da sua marca nos navegadores e redes sociais, configure também o <a href="<?= dash_url('media/favicons') ?>">Favicon</a> (o sistema gerará os ícones e o arquivo manifest para PWA) e o <a href="<?= dash_url('media/poster') ?>">Poster</a> de Compartilhamento (imagem fallback para redes sociais)</p>
</div>