...
if( $_POST['action'] === 'favicon' ) {
    ...
    if( ! FILES::mime('favicon', 'image/png') ) {
        alert('warning discard', 'Esse arquivo precisa ser uma imagem <strong>PNG</strong>');
    ...
    foreach( $dimensions as $scope => $size ) {

        $resolved = ImageHandler::resolve([
            'input'    => 'favicon',
    ...
    if( ! $resolved ) {
        alert(
            'warning', 
            'Não foi possível gerar os ícones do site.<br>
             Verifique se o arquivo enviado tem o tipo mime <code>image/png</code> e tente novamente.'
        );
    ...
    if( file_exists(UPLOAD_DIR . 'favicons/192x192.png') ) {
        ...
        require_annex('manifest-json.php');
    }
}
