...
if( $action === 'insert' ) {
    ...
    if( ArrayExport::apply( $array_name, $array_data, 'menus') ) {
        alert_redirect(
            ...
            dash_url("menus/?key={$array_data['key']}"), 3000
        );
    }
    else {
...
if( $action === 'update' ) {
    ...
    if( $updated ) {
        alert_redirect(
            ...
            dash_url("menus/?key={$new_var}"), 3000
        );
        exit;
    }
    else {
...
if( $action === 'delete' ) {
    ...
    if( Menu::delete($current_menu) ) {
        alert_redirect( 'success', 
            "Menu <strong>{$current_menu}</strong> excluído", dash_url("menus") 
        );
    ...
    if( Cache::deleteFile("menus/{$current_menu}.html") ) {
        ...
        alert(
            ...
            sprintf(
                ...
                site_url(),
                dash_url("menus/?key={$current_menu}" )
            ),
            URL::current()
        );
    }
