...
class SitemapXML {
    ...
    private function render() {
        $xsl_url = URL::root('web/callable/sitemap.xsl');
    ...
    private function categories(): string {
        ...
        if( $row ) {
        	...
        	# todas as categorias 
            # as mesmas que listam posts ex: 'https://domain.ext/categorias/1/2/max3'
        	do {
        		$xml .= $this->build_URL( 
    ...
    private function posts(): string {
        $cmd = $this->conn->query("
            SELECT slug, updated FROM posts WHERE status = 1 ORDER BY ID DESC
        ");
        $row = $cmd->fetch(PDO::FETCH_ASSOC);

        $xml = '';
        if( $row ) {
            # pagina da listagem de posts 
