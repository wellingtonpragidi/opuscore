...
class Htaccess {
    ...
    private string $start_marker = '# INICIO das diretivas geradas pelo sistema';
    private string $end_marker   = '# FIM das diretivas geradas pelo sistema';
    ...
    public function rewrite(): void {
        ...
        $has_start = str_contains( $current, $this->start_marker );
        $has_end   = str_contains( $current, $this->end_marker );

        # Regex que captura tudo entre os marcadores (incluindo os proprios marcadores)
        $start_escaped = preg_quote( $this->start_marker, '/' );
        $end_escaped   = preg_quote( $this->end_marker, '/' );
        ...
        $has_start = str_contains( $cleaned, $this->start_marker );
        $has_end   = str_contains( $cleaned, $this->end_marker );
        ...
        if( $has_start XOR $has_end ) {
            ...
            if( $has_start ) {
                $cleaned = preg_replace(
                    '/' . preg_quote($this->start_marker, '/') . '.*/s', '', $cleaned
                );
            } 
            else {
                $cleaned = preg_replace(
                    '/.*' . preg_quote($this->end_marker, '/') . '/s', '', $cleaned
                );
            }
    ...
    private function markers(): string {
        ...
        return
            $this->start_marker 
            . "\n# A edição manual entre \"# INICIO e # FIM\" deste arquivo serão sobrescritas\n" 
            . $this->directives() 
            . "\n" 
            . $this->end_marker;
    }
    ...
    private function directives(): string {
        return IS_LOCAL 
            ? $this->local_directives() 
            : $this->production_directives();
    }
