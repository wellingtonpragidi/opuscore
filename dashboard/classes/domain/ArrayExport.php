...
class ArrayExport {
    ...
    private static function write( string $file, array $vars ): bool {
        ...
        $filetmp = tempnam( dirname($file), 'opus_' );
        ...
        if( Ensure::writeLock( $filetmp, $content ) === false ) {
            unlink( $filetmp );

            return false;
        }
        
        # Move o arquivo temporario para o arquivo final (operacao atomica)
        $written = rename( $filetmp, $file );
    ...
    private static function array_merge_deep( array $defined_var, array $array_data ): array {
        ...
        foreach( $array_data as $key => $value ) {
            $is_dimensional = array_key_exists( $key, $defined_var ) 
