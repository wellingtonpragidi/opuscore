...
class Category extends Model {
    ...
    public function select(): array {
        ...
        if( URL::has('id') ) {
            $cmd = $this->conn->prepare("
                SELECT c.*, m.attachment FROM categories c 
                LEFT JOIN medias m ON m.related_id = c.ID AND m.related_type = ? 
                WHERE c.type = ? AND c.ID = ?
            ");
            $cmd->execute([ 
                'category-article', # tipo da midia (imagem) relacionada
                'article',          # tipo da categoria
            ...
            $cmd = $this->conn->prepare(
                ...
                WHERE c.type = ?
            ");
            $cmd->execute([ 'category-article', 'article' ]);
        }
    ...
    /**
     * Esse delete faz SELECT antes 
     *   para verificar se o registro sendo excluido possui sucessores
     * E: 
     * Caso tenha o controller interrompe o processo
     * Do contrario (`false`) executa o DELETE
     */
    public function delete( Assign $bind ): array {
        ...
        $has_parent = (int) $cmd->fetchColumn() > 0;
    ...
    public function select_table(): string {
        ...
        # Busca todas as categorias para o tipo especificado na URL, ordenadas por pai e nome.
        $cmd = $this->conn->prepare("
            SELECT c.*, m.attachment 
            FROM categories c 

            LEFT JOIN medias m 
                ON m.related_id = c.ID AND m.related_type = ? 
        ...
        while( ! empty($stack) ) {
            ...
            $attachment = Ensure::object( $cat['attachment'] );
            $thumbnail  = $attachment->thumb->path ?? null;

            //var_dump( [$cat["ID"], $cat["name"], ($attachment->thumbs ?? '')] );
            $fallback   = ' fallback';
            if( isset($thumbnail) ) {
            ...
            '<form method="POST" action="' . URL::current() . '">
                <tr>
                    ...
                    <td>
                        <button 
                            onclick="javascript: return confirm(`Vai mesmo excluir esta categoria?\n\nNão será possível completar essa ação caso a mesma possua sucessores.`)"
                            class="input_false link delete fs23 txt_center" 
                            type="submit" name="action" value="delete"
                        >
                            <span icon="close" size="26" top="3"></span>
                        </button>
                        <input type="hidden" name="parent" value="' . $cat["parent"] . '" />
    ...
    public function update_rebuild_segment( int $bindID ): string {
        ...
        while( ! empty($queue) ) {
            ...
            $cmd = $this->conn->prepare("SELECT ID FROM categories WHERE parent = ?");
            $cmd->execute([ $id ]);
