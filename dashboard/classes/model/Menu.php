<?php
/**
 * Classe Menu - Gerencia a criacao, edicao, exclusao e exibicao de menus de navegacao.
 *
 * Esta classe eh responsavel por interagir com as tabelas `contexts` (para nomes de menus)
 * e `menu` (para itens de menu), alem de outras tabelas para obter conteudos vinculados
 * (paginas, categorias). Ela oferece metodos para:
 * - Exibir menus no frontend (`nav_list`).
 * - Gerenciar menus e seus itens no painel de controle (create, update, delete, order).
 * - Renderizar a estrutura de edicao de menu no painel.
 * - Obter titulos e URLs de paginas e categorias.
 * - Listar itens de paginas e categorias disponiveis para adicao ao menu.
 * 1.2.0
 * 
 * @package System/Builders
 * @subpackage Model
 */
...
class Menu {
    ...
    public static function add_checked_items( 
        ?string $menu_name, array $items, int $parent = 0 ): void {
        ...
        foreach( $items as $index => $item ) {
            ...
            $cmd->execute([ $menu_name, $parent, $type, $related, $label, $url, $sort ]);

            # Se o item tiver filhos, chama a funcao recursivamente para inserir os filhos.
            if( isset($item['children']) && is_array($item['children']) ) {
                self::add_checked_items( 
                    $conn, $menu_name, $item['children'], $conn->lastInsertId() 
                );
            }
        }
    }
     ...
     * E por fim exclui o item: DELETE
     */
     ...
     *
     * @uses URL::Get() Para obter o nome do menu da URL.
     * @return int O numero de itens encontrados para o menu especificado.
     */
    ...
    public static function list_category_items(): string {
        ...
        $cmd = self::$conn->prepare("
            ...
            FROM categories WHERE type = 'post' ORDER BY parent, name ASC
        ");/*, slug*/
        $cmd->execute();
    ...
    private static function get_entity_title( int $id, string $table ): string {
        self::init();
        $table  = ($table === 'categories') ? 'categories' : 'pages';
        $column = ($table === 'categories') ? 'name' : 'title';
     ...
     * 
     * @todo Necessita condicao hierquica para URLs de posts category/slug e pages parent/page
     */
    private static function get_entity_url( int $id, string $table ): string {
        self::init();
        $column = $table === 'category' ? 'segment' : 'slug';
        $cmd = self::$conn->prepare("SELECT $column FROM $table WHERE ID = ?");
        $cmd->execute([ $id ]);

        $fetch = $cmd->fetchColumn() ?: '';
