...
class PHPMailer {
     ...
     * This length does NOT require the line break
     ...
     * The default boundaries require a "=_" sequence which cannot occur in quoted-printable bodies,
    ...
    {
        //It's not possible to use shell commands safely (which requires the mail() function) without escapeshellarg,
     ...
     */
    public function setLanguage( $langcode = 'pt-br', $lang_path = '' ) {
        $this->language = [
            /**
            ...
            'buggy_php'            => 'Sua versão do PHP é afetada por um bug que por resultar em messagens corrompidas. Para corrigir, mude para enviar usando SMTP, desative a opção mail.add_x_header em seu php.ini, mude para MacOS ou Linux, ou atualize seu PHP para versão 7.0.17+ ou 7.1.3+ ',
            'connect_host'         => 'Erro de SMTP: Não foi possível conectar ao servidor SMTP.',
            'data_not_accepted'    => 'Erro de SMTP: Dados rejeitados.',
            'empty_message'        => 'Mensagem vazia',
            'encoding'             => 'Codificação desconhecida: ',
            'execute'              => 'Não foi possível executar: ',
            'extension_missing'    => 'Extensão não existe: ',
            'file_access'          => 'Não foi possível acessar o arquivo: ',
            'file_open'            => 'Erro de Arquivo: Não foi possível abrir o arquivo: ',
            'from_failed'          => 'Os seguintes remetentes falharam: ',
            'instantiate'          => 'Não foi possível instanciar a função mail.',
            'invalid_address'      => 'Endereço de e-mail inválido: ',
            'invalid_header'       => 'Nome ou valor de cabeçalho inválido',
            'invalid_hostentry'    => 'hostentry inválido: ',
            'invalid_host'         => 'host inválido: ',
            'mailer_not_supported' => ' mailer não é suportado.',
            'provide_address'      => 'Você deve informar pelo menos um destinatário.',
            'recipients_failed'    => 'Erro de SMTP: Os seguintes destinatários falharam: ',
            'signing'              => 'Erro de Assinatura: ',
            'smtp_connect_failed'  => 'SMTP Connect() falhou.',
            'smtp_code'            => 'Código do servidor SMTP: ',
            'smtp_error'           => 'Erro de servidor SMTP: ',
            'smtp_code_ex'         => 'Informações adicionais do servidor SMTP: ',
            'smtp_detail'          => 'Detalhes do servidor SMTP: ',
            'variable_set'         => 'Não foi possível definir ou redefinir a variável: '
        ];
        return true;
    }
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] === $disposition_type) {
                ...
                //Only require a filename property if we have one
                if (!empty($name)) {
                    $mime[] = sprintf(
     ...
     * Picks shortest of Q, B, or none. Result requires folding if needed.
     ...
     * This can require images, sounds, and just about any other document type.
     ...
     * IPv6 addresses must require [], e.g. `[::1]`, not just `::1`.
     *
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
        ...
        foreach ($parsedHeaders as $header) {
            //Is this header one that must be required in the DKIM signature?
        ...
        //The DKIM-Signature header is required in the signature *except for* the value of the `b` tag
     ...
     * Before the send() call, queued addresses (i.e. with IDN) are not yet required.
     *
     * @return array
     */
