...
class Menu {
    ...
    public static function list( array $args = [] ): void {
        ...
        if( ($html = Cache::getFile($file_cached)) !== null ) {
            ...
            $html = str_replace( '{{AUTH_ITEM}}', self::render_auth_item($label), $html );
     ...
     *
     * @param array $items Uma arvore de itens de menu, geralmente gerada por `build_tree()`.
     * @param array $args Argumentos de configuracao para classes CSS (list_id, list_class, sublist_class, parent_item_class).
     * @param int $depth Nivel de profundidade atual na recursao 
     * @return string O HTML renderizado da lista de menu.
     */
    ...
    private static function render_auth_item( string $label ): string {
        $us = Container::call('UserStatus');
        if( $us->is_logged() ) {
            return '<li class="menu-item-auth logged">
                <a href="' . $us->logged('url') . '">' . $us->logged('name') . '</a>
            </li>';
        }

        return 
        '<li class="menu-item-auth">
            <a href="' . site_url('access/?action=login') . '">' . $label . '</a>
        </li>';
    }
