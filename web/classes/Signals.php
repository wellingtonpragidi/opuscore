...
class Signals {
    ...
    public function routes(): ?string {
        if( is_home() ) {
            ...
            return $this->indexed( $this->post->meta_description(), 'wide' );
        }
        else if( is_posts() ) {

            return $this->indexed( seo('posts_description'), null );
        }
        else if( is_user() ) {
    ...
    private function meta_twitter( string $description, ?string $scope ): string {
        ...
        elseif( file_exists(UPLOAD_DIR . 'favicon/poster.png') ) {
            $emit .= '<meta name="twitter:card" content="summary_large_image" />' . PHP_EOL;
