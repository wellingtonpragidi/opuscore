 ...
 * associacao com posts e recuperacao de informacoes de categorias.
 *
...
class Category extends Model {
     ...
     * Inicialmente com 'post', mas pode ser extendido.
     */
    private array|string $params = ['post'];
    ...
    public function select( array $except = [] ): array {
        ...
        $cmd = $this->conn->prepare("
            ...
            FROM categories c 
            LEFT JOIN medias m 
            ON m.related_id = c.ID AND m.related_type = 'category-post' 
            {$WHERE}
        ...
        while( ! empty($stack) ) {
            ...
            if( ! empty($hierarchy[$bind->ID]) ) {
                ...
                $bind->dynamo['class'] = 'category-parent has-child';

                # (tem filhos) — e tmb eh filho (tem pai)
                if( $bind->parent !== 0 ) {
                    $bind->dynamo['class'] = 'category-child has-child child-' . $level;
                }
            }
            # aqui categorias nao tem filhos
                ...
                if( $bind->parent === 0 ) {
                    $bind->dynamo['class'] = 'category-parent';
                }
                # por fim: (nao tem filhos) — eh filho (tmb eh neto) - tem pai e avo
                else {
                    $bind->dynamo['class'] = 'category-child child-' . $level;
                }
            }
    ...
    public function list( array $args = [] ): string {
        ...
        $cmd = $this->conn->prepare("SELECT segment FROM categories WHERE type = 'post'");
        $cmd->execute();
        $cats_segment = [];
        ...
        $post_cats_segment = [];
        foreach( $this->post_cats() as $row ) {
            $post_cats_segment[] = $row['segment'];
        }

        if( $is_post ) {
            $compare = array_diff( $cats_segment, $post_cats_segment );
        }
        ...
        while( ! empty($stack) ) {
            ...
            # @docx | adicionado class=last-item
            $is_last = empty( $stack );
            ...
            if( $thumbnail && $thumbnail === 'display' || $thumbnail === true ) {
                ...
                if( $attachment && isset($attachment->thumb->path) ) {
                    ...
                    $alt = 'alt="' . $attr_name . '"';
                    $dimensions = Image::dimensions_attrs( $attachment->thumb ?? null );
    ...
    # Retorna um array com ID, nome e segmento de categorias relacionadas (marcadas) por um post 
    public function post_cats(): array {
        $cmd = $this->conn->prepare("
            SELECT c.ID, c.name, c.segment
            FROM categories c
            JOIN relations r ON c.ID = r.category_id
            JOIN posts p ON p.ID = r.type_id
            WHERE p.segment = ? AND c.type = ? AND r.type = ?
            ORDER BY c.parent ASC, c.name ASC
        ");

        $cmd->execute([ URL::pathname(), 'post', 'post' ]);
