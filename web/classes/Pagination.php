 ...
 * - calcular o offset para paginação de articles
 ...
 * 
 * usa funcoes auxiliares como articles_per_page() e url::param() para trabalhar com parametros da url
 * 
...
class Pagination {
    ...
    /**
     * calcula o offset para consulta dos articles baseado no parametro 'pg' da url
     * se 'pg' nao estiver definido ou for 1, retorna 0 (primeira pagina)
     *
     * @return int offset para usar em consulta sql (limit offset, articles_per_page())
     */
    public function article_paginate() {
        $iG = isset($_GET['pg']) ? $_GET['pg'] : 1;
        return ( $iG == '' || $iG == 1 ) ? 0 : ( $iG * articles_per_page() - articles_per_page() );
    }

    /**
     * gera o html da navegacao paginada para articles
     ...
     * @return string
     */
    public function article_paginator() {
        $article = Container::call('Article'); 
        $GETpg = URL::Get('pg');
        $number_records_rounded = ceil( $article->total_records() / articles_per_page() );
        ...
        if( articles_per_page() < $article->total_records() ) {
            $html = '';
